...
from pire_dirt.engine import run_training
...
def main() -> None:
    parser = build_common_parser("Distributed training for PIRE-DIRT")
    ...
    try:
        ...
        if is_main_process():
            ...
            print(
                f"[PIRE] enabled={model.use_pire}, "
                f"strength={config['model']['pire_strength']}"
            )
            print(
                f"[DIRT] retain_ratio={config['model']['dirt_retain_ratio']}"
            )
