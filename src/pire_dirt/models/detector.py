...
class DINOv3LoRAPIREDIRT(nn.Module):
    """DINOv3 detector equipped with LoRA, PIRE, and DIRT."""
    ...
    def __init__(
        ...
        lora_rank: int = 2,
        lora_alpha: int = 2,
        dirt_hidden_dim: int = 256,
        dirt_retain_ratio: float = 0.10,
        use_pire: bool = True,
        pire_strength: float = 0.20,
    ) -> None:
        super().__init__()
        self.use_pire = use_pire
        ...
        if not checkpoint_path.is_file():
            raise FileNotFoundError(
                f"Backbone checkpoint does not exist: {checkpoint_path}"
            )
        ...
        self.lora_layer_count = add_lora_to_dinov3(
            self.backbone,
            rank=lora_rank,
            alpha=lora_alpha,
        )
        ...
        self.dirt = DIRT(
            ...
            hidden_dim=dirt_hidden_dim,
            retain_ratio=dirt_retain_ratio,
        )
        self.pire = PIRE(
            embed_dim=embed_dim,
            strength=pire_strength,
        )
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(tokens, dict):
            if "x_norm_clstoken" in tokens and "x_norm_patchtokens" in tokens:
                ...
                raise KeyError(
                    "Unsupported dictionary output from backbone.forward_features"
                )
        else:
            cls_token = tokens[:, 0]
            patch_tokens = tokens[:, 1:]

        if self.training and self.use_pire:
            patch_tokens = self.pire(patch_tokens)

        global_logits = self.cls_head(cls_token)
        regional_logits = self.dirt(patch_tokens)
